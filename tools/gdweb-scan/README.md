...
CSV에는 **최종 URL** 열이 있습니다 — 리다이렉트가 일어났을 때만 채워지므로,
목록의 주소가 엉뚱한 곳으로 가고 있지 않은지 확인할 수 있습니다.

### 실패 자동 재시도

1차 측정이 끝나면 **재시도 패스**가 돕니다. 일부 사이트는 헤드리스 크롬의 HTTP/2
협상에서 넘어지는데(`ERR_HTTP2_PROTOCOL_ERROR`), 이건 차단이 아니라 프로토콜
문제라 **HTTP/1.1로 내려 한 번만 다시 시도**합니다. GDWEB 50건 스캔에서 대한항공이
이 경우였습니다.

재시도 대상은 일시적 실패로 보이는 것만입니다 — HTTP/2 오류, 연결 리셋, 타임아웃.
404 같은 확정 실패는 재시도하지 않습니다. 재시도로 살아난 항목은 CSV 상태 열에
`측정됨 (HTTP/1.1 재시도)`로 표시됩니다.
